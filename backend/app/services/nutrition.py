...
def _build_recommendations(
    ...
    if day_strain is not None:
        if day_strain >= 14 and goal != "gain":
            recs.append({
                "type": "protein",
                "icon": "💪",
                "text": "Високе навантаження сьогодні — додайте білка для збереження м'язів",
            })
        if day_strain >= 16 and goal == "lose":
            recs.append({
                "type": "carbs",
                "icon": "⚡",
                "text": "Дуже високе навантаження на дефіциті — додайте вуглеводів для відновлення",
            })
        if day_strain < 5 and goal == "gain":
            recs.append({
                "type": "training",
                "icon": "🏋️",
                "text": "Низьке навантаження при наборі маси — збільшіть інтенсивність тренування",
            })
        if day_strain >= 10:
            recs.append({
                "type": "water",
                "icon": "💧",
                "text": "При навантаженні вище 10 пийте не менше 3 л води",
            })
    ...
    if goal == "lose" and target_calories < 1500:
        recs.append({
            "type": "warning",
            "icon": "⚠️",
            "text": "Калорійність нижче 1500 — не рекомендується тривалий дефіцит",
        })
